...
Everdice uses a React frontend with TypeScript, Wouter for routing, TanStack React Query for state management, shadcn/ui (Radix UI) for components, and Tailwind CSS for styling. The backend is an Express.js application (TypeScript) with Drizzle ORM for PostgreSQL (Neon serverless). Authentication uses Passport.js, and real-time features use WebSockets. AI integration uses a centralized provider abstraction (`server/lib/aiProvider.ts`) that supports user-configurable LLM services (OpenAI, Anthropic, local/self-hosted, or any OpenAI-compatible API) with automatic fallback to the app's OpenAI GPT-4o subscription. DALL-E 3 portrait generation always uses the app's OpenAI key. User LLM configs are stored in the `llm_configs` table and managed via the AI Settings page (`/ai-settings`).
...
3. *Chapter Progression Nudging*: After 5 scenes in the same chapter → gentle nudge. After 8 → moderate urgency ("converge within 2-3 scenes"). After 11 → urgent ("resolve NOW"). Scene count uses actual sessions since last chapter gate (tracked via narrativeLog entries with type 'chapter_gate'). Applied in both advance-story routes (simple and authenticated). Hard-cap failsafe: after 12 sessions without gate met, server forces chapter advancement.

**Chapter Tracking Architecture** (Feb 2026): The `currentSession` field on campaigns tracks the CHAPTER number, NOT the session count. Session counting uses the actual count of campaign_sessions rows. Chapter advancement ONLY happens via (1) AI returning `chapterGateMet` in its response, or (2) hard-cap failsafe after 12 sessions in one chapter. The simple advance-story route no longer auto-increments `currentSession` — it only saves sessions and lets chapter gates drive progression. The AI prompt's "0D" rule makes the chapter gate the PRIMARY narrative directive.
