...
### DM Authoring Doctrine
Stakes-driven campaign system where every choice costs, closes, or escalates. Schema fields: `campaignQuestion` (one-sentence spine), `campaignStakes` (2-4 stakes, 0-5 scale), `chapterGates` (meaning-based advancement), `narrativeLog` (structured logging). Campaign generation AI prompt enforces doctrine: no free actions, combat as consequence not content. Advance-story AI responses include `campaignStakeUpdates`, `chapterGateMet`, and `narrativeLogEntry`. Chapter advancement uses meaning-based gates (truths learned, beliefs changed, commitments made) instead of metrics/turn counts. Both advance-story handlers (`/api/campaigns/advance-story` and `/api/campaigns/:campaignId/advance-story`) process and persist doctrine updates. The legacy metrics-based chapter advancement still runs as a fallback alongside gate-based advancement.

**Doctrine Auto-Improvisation:** The `improviseDoctrine()` function (server/routes.ts) detects campaigns missing doctrine fields and auto-generates them from campaign metadata. It uses keyword-based theme detection (undead, wilderness, political, arcane, heist, horror, nautical, divine, exploration) to select appropriate stake templates and campaign questions. Generated doctrine is persisted to the database on first use so improvisation only runs once per campaign. This ensures imported CAML adventures and pre-existing campaigns benefit from stakes-driven storytelling without requiring manual doctrine setup.
