...
Trek System (`trek_routes` table, with `pending_encounter` JSONB column, `originQ`/`originR` fields, `lootFound` JSONB array) allows players to set a destination hex on the world map and travel toward it step by step. The trek path is computed and visualized as a dashed golden line on the hex map. Each step has a 25% chance to trigger enriched encounters (ambush, traveler, discovery, weather, wildlife, loot_find) with descriptions, hooks, scene types, and narrative categories. The "loot_find" encounter type generates random items (weapons, potions, gear) with rarity/value/effects and bonus gold. When an encounter triggers, the trek status changes to "encounter" and the encounter data is persisted in `pending_encounter`. Players can "Enter Narrative" to generate AI-powered D&D scenes or "Pass By" to dismiss. Trek auto-return: saves origin hex at trek start, automatically returns party to origin on completion/cancel. Backend endpoints: `POST /api/campaigns/:id/trek/start`, `POST .../trek/step`, `GET .../trek/active`, `POST .../trek/cancel`, `POST .../trek/enter-narrative`, `POST .../trek/dismiss-encounter`.

Capital City System: "Crownhaven — The Capital" (location_type="capital") uses a special `generateCapitalCityLayout` function producing 8 districts (Royal Quarter, Grand Market, Temple Row, Thieves' Quarter, Harbor Ward, Artisan Heights, Scholar's Enclave, Old City) with 24+ named buildings, 4 city gates, and named streets. Capital-specific building types include palace, bank, real_estate, dark_temple, auction, academy, dungeon_entrance, information_broker, tailor. Interactive Bank system (`player_bank` table): deposit/withdraw gold with 1% daily interest, transaction history. Interactive Housing system (`player_houses` table): buy/sell houses across 5 tiers (200gp modest to 15,000gp manor), store items in your home. Bank and housing panels render inline in the CityMap sidebar when those building types are selected.
